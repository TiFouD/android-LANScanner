...
class NetworkScanner(private val context: Context) {
    ...
    suspend fun startScan(): List<DeviceInfo> {
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (deviceIp == null) {
                Log.e(TAG, "Error: Could not find device IP. Is Wi-Fi enabled?")
                return@withContext emptyList<DeviceInfo>()
            }

            val deviceHostname = InetAddress.getByName(deviceIp).hostName
            val deviceIpInfo = DeviceInfo(deviceIp, if (deviceHostname == deviceIp) "Your Device" else deviceHostname)
            devices.add(deviceIpInfo)


            val subnet = deviceIp.substring(0, deviceIp.lastIndexOf('.'))
            val deviceIpLastOctet = deviceIp.substring(deviceIp.lastIndexOf('.') + 1).toInt()
            ...
            val jobs = (1..254).map { i ->
                if (i == deviceIpLastOctet) return@map null
